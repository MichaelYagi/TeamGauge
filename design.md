...
Both providers share the same JSON Schema objects, but Claude's structured-output validator is the stricter of the two: it requires `additionalProperties: false` on every `object`-type node, nested ones included, and 400s on the whole request otherwise — Ollama doesn't enforce this. A schema built and tested against Ollama alone can pass every test and still fail the moment Claude is tried; see claude.md's Reasoning Step section for the specific fix.

Claude's response can also be cut off before the JSON finishes if the analysis it's asked for is long enough — a real team of 9 engineers exhausted a 4096-token budget mid-string, surfacing as an opaque `JSON.parse` "Unterminated string" error rather than anything explaining what actually happened. `ClaudeReasoningProvider` uses a larger budget (8192) and checks the response's `stop_reason` before parsing, turning a truncation into a clear error instead of a confusing crash.
...
Comparative and specific is not automatically readable — an early version of this prompt got exactly what it asked for (named comparisons, ratios) but produced sentences with several statistics stacked into one clause, which reads as a data dump rather than a note a manager can act on. Both prompts now explicitly require plain, one-idea-per-sentence writing that leads with what a finding means rather than the arithmetic behind it, without weakening the "must be comparative and named" requirement — see claude.md's Reasoning Step section for the concrete before/after.
