...
**One provider/model setting for the whole app, not one per reasoning form.** The UI has (at least) four separate reasoning entry points — the per-report "Generate Recommendations" form, the History & Trends Accumulated Report, its per-person report, and the Sprint Report panel — and an earlier version of this UI gave each of the first three its own independent Provider/URL/Model fields (the fourth, Sprint Report, was *added* with its own picker to fix a real gap: it had silently reused the Accumulated Report's fields with no visible indication, so a user working from Sprint Report had no way to tell it was even using a provider, let alone change it). Both of those were wrong in different directions — no picker at all, and a picker per form — and the user rejected the per-form fix directly: *"you're making it too complicated, 1 model for everything, don't split it up like that."* The fix that shipped: a dedicated **Settings** tab (`web/index.html`'s `#settings-panel`) with exactly one Provider/URL/Model control, saved via `getReasoningSettings()`/`saveReasoningSettings()` (`web/app.js`) to `localStorage` (a personal reasoning preference, not team data, so it doesn't need a DB column or to sync across browsers/devices) — and it is the *only* provider/model UI in the app. All four reasoning entry points call `getReasoningSettings()` at request time and have no provider/model fields of their own; each keeps its own free-text **context** field, since that content is genuinely different per use case (a per-sprint note vs. a whole-history note), unlike the provider/model choice, which is the same regardless of which report you're generating.
...
- **`--force` cannot create a byte-identical duplicate — no override exists for this.** The collision guard above only ever checked *identity* (team+sprint+date), not *content*, which left a real hole: clicking through "Save as new snapshot" (the UI's non-blocking collision notice) or passing `--force` always saved, even when the new report was exactly the same as what was already there — a real production case produced two rows with identical `sprint`, `snapshot_date`, and byte-identical `report_json`, differing only by `id`/`created_at`. Before saving under `--force`/`force: true`, both the CLI and `POST /api/analyze` now compare `JSON.stringify(report)` against the colliding snapshot's `report_json`; if identical, the save is refused (not just warned about) with the existing snapshot's id/timestamp, since an exact duplicate carries zero new information and only pollutes `trend` with a meaningless zero-change row. This is a hard rule, not a suggestion the user can override — a genuine re-check always produces at least some different signal (even "load_score is 0.1 higher"), so identical-to-the-byte is a strong enough signal of "this shouldn't be a new row" to refuse unconditionally.
- **Sprint label drift detection.** `findSimilarSprint` (`src/db/snapshots.ts`) checks a newly resolved sprint label against every sprint label already on record for that team (`listKnownSprints`); if the new label isn't an exact match but one contains the other as a substring (case-insensitive), it's surfaced as a non-blocking warning before saving — both `analyze --team` (stderr notice) and `POST /api/analyze` (`similarSprintWarning` in the response, rendered by the UI in its own warning box below the Analyze form). This exists because sprint identity can silently fragment: one analyze run resolves a sprint as `"26.18"` (typed by hand, or a source that only carries the short form) and a later run on the same real sprint auto-detects the fuller `"BDPSA Sprint 26.18"` (the literal name from a CSV's Sprint column) — same sprint, two labels, and `history`/`trend` from then on track them as two unrelated sprints with no indication anything went wrong. The check is a plain substring match, not a general fuzzy matcher — it catches this specific, observed shape of drift (one label is the other plus a prefix/suffix) without trying to solve arbitrary "are these the same sprint" similarity.
- **Merging drifted sprint labels.** The warning above is preventive — it doesn't fix labels that already drifted apart before the check existed. `teamgauge snapshot rename-sprint --team <name> --from <label> --to <label>` (`renameSprintLabel` in `src/db/snapshots.ts`) merges two labels by renaming every snapshot with `--from` to `--to`, so `history`/`trend` see one continuous sprint instead of two fragments. It refuses (rather than silently overwriting) if the rename would land two snapshots on the same team+sprint+date — that's the same ambiguity the exact-collision guard exists to prevent, so this doesn't get a way around it either. `POST /api/snapshots/rename-sprint` is the server equivalent; `GET /api/teams/profile` now also returns `knownSprints` (from `listKnownSprints`) so the UI can offer a picker instead of requiring a typed label. The Team Setup panel has a "Merge Sprint Labels" control (two `<select>`s populated from the selected team's known sprints, a confirm-guarded "Merge" button) — this was initially built CLI-only on the assumption that merging sprint identity is rare enough not to need a UI surface, but the user asked for it through the UI, and every other administrative action in this app already has one — CLI-only was the inconsistent choice.
...
- `reasonAboutTrend` (`src/reasoning/cumulativeReason.ts`, `CUMULATIVE_JSON_SCHEMA` in `src/reasoning/cumulativeSchema.ts`) reasons over a whole `TeamTrend` (every snapshot's points *and* deltas, from `computeTrend`) and produces a **new, freestanding, structured** report — never a per-snapshot echo.
- `reasonAboutPersonTrend` is the same idea scoped to one person's own `EngineerTrend` (`PERSON_CUMULATIVE_JSON_SCHEMA`/`PersonCumulativeReportOutputSchema`) — a genuinely different, smaller schema from the team-wide one (see below), not that schema reused with less data.
- Both paths share the same underlying provider mechanism: `ReasoningProvider` (`src/reasoning/types.ts`) now exposes a generic `chat(prompt, jsonSchema): Promise<unknown>`, which `reason()` and `reasonAboutTrend()`/`reasonAboutPersonTrend()` both call with their own prompt-builder and schema — `OllamaReasoningProvider`/`ClaudeReasoningProvider` implement `chat()` once; `reason()` is just `chat()` plus the Unassigned-stripping and validation already documented above.
...
**Team-wide cumulative output is structured into sections, not one narrative blob** — `{ summary, team_overview: {throughput, cycle_time, stability}, engineer_patterns: {overloaded, underutilized, stable_anchors}, unassigned_risk, overall_assessment, leadership_takeaways: {root_causes, opportunities}, concerning_trends }`. This shape was chosen over a single long write-up specifically so "who's overloaded" is a real, reusable list the UI can render as consistent sectioned cards every time, not prose a person has to re-read to extract structure from — the user's own words requesting this: a pasted example report with exactly these categories ("🔴 overloaded," "🟢 underutilized," "🟡 stable anchors," "⚫ unassigned = systemic dysfunction," a leadership root-causes/opportunities split) as the bar this app's Accumulated Report should hit. `engineer_patterns` entries are `{name, reason}`, not bare strings, so a name can be validated separately from the free-text reasoning next to it. `unassigned_risk` is `""` (never `null`) when there's no Unassigned bucket or nothing notable — an empty-string sentinel, consistent with `engineer.role`'s "" meaning unknown elsewhere in this schema, chosen specifically because JSON Schema's nullable-type support isn't something both providers' structured-output validators are known to handle identically, and this app has already been burned once by an unvalidated cross-provider schema assumption (see the Claude `additionalProperties` note above).

**`team_overview.throughput`/`cycle_time` carry an explicit diagnostic layer, not just a description of what happened.** After citing the evidence, each field adds a line starting "This usually means:" followed by 3-5 concrete, plausible causes (inconsistent intake, unstable prioritization, engineers frequently blocked, inconsistent sizing, reacting instead of executing for throughput; work stuck, oversized items, dependency waits for cycle time) — matching the exact pattern in the user's pasted reference example, which named a phenomenon, cited evidence, then explicitly listed what a pattern like that "usually means." `team_overview.stability` similarly must quote 2-3 specific consecutive transitions verbatim with explicit good/bad framing (e.g. "26.7 -> 26.8: velocity +24 ... a strong sprint" immediately followed by "26.8 -> 26.9: velocity -12.5 ... a regression") and name the repeating cycle those transitions belong to, rather than describing the pattern only in the abstract.

**`overall_assessment` is the connecting synthesis the other sections deliberately don't repeat** — the equivalent of the user's example's "What this means for the team overall" section, written as the conclusion a manager reading the whole report end to end would draw, required to cover planning/sizing discipline, execution efficiency, how load is actually distributed, and whether the team reads as executing a plan or reacting sprint to sprint, citing something already established elsewhere in the same report for each point (never a fresh, uncited claim).

**A structural limit worth knowing: `formatMessage` (the shared markdown renderer, loaded externally — see the UI Requirements section above) never produces real `<ul><li>` markup for a markdown list embedded inside one string field** — verified directly: `formatMessage("- a\n- b")` renders as `a<br>&bull; b` (bullet-character + line-break), not a semantic list, regardless of whether the source uses `-` or a literal `•`. This only affects fields where the model embeds a list inside free text (`team_overview`/`overall_assessment`) — the app's own hand-built lists (`recommendations`, `concerning_trends`, `leadership_takeaways.root_causes`/`.opportunities`, and every other array-backed section) wrap each array item in a real `<li>` in `web/app.js` itself and are unaffected. The visual result (a bullet character starting its own line) is still legible as a bulleted list; a `normalizeBullets()` pre-pass converting `•` to `-` before calling `formatMessage` was tried and reverted after confirming empirically that it changes nothing — `formatMessage` treats both identically. Getting true `<ul><li>` for these two fields would require restructuring them from a free-text string into an array (i.e. splitting the narrative and its causes into separate fields), which was deliberately not done here — the user's priority, in their own words, was report depth/content ("I don't care about accuracy, I care about the output"), and the current rendering is already readable.

**Fabricated/mis-cased engineer names in `engineer_patterns` are stripped deterministically, not just prompted against.** A real, observed failure caught in this feature's own first live verification: a smaller local Ollama model named `"You01"` (not a real engineer anywhere in the data) and `"Kenneth.vanderlinde"` (wrong casing of the real `"kenneth.vanderlinde"`) in `engineer_patterns`, despite the schema's field description saying a name "must exactly match an engineer name from the input data." Same category of gap as the Unassigned-stripping rule above — prompting alone isn't reliable enough on smaller models — so `reasonAboutTrend` now filters every `overloaded`/`underutilized`/`stable_anchors` entry against the real set of names in the `TeamTrend` it was given (`keepOnlyRealEngineers`), dropping anything that doesn't match exactly, after Zod validation and before the result reaches the caller.

**Per-person cumulative output is a different, smaller shape, not the team schema minus fields**: `{ summary, trajectory_overview: {workload, cycle_time, stability}, pattern: {classification, reason}, recommendations, concerning_trends }`. `engineer_patterns` (categorizing *other* people) and `unassigned_risk`/`leadership_takeaways` (team-level backlog/process concepts) don't apply to one person's own history, so they're absent entirely rather than left empty — `pattern.classification` is a single enum (`overloaded | underutilized | stable | improving | declining | volatile`) naming the one best-fitting label for that person's trajectory as a whole.

`POST /api/trend-reason` (`{ team, sprint?, provider?, url?, model?, context? }`) and `POST /api/trend-reason-person` (adds `engineerName`) are the server-side equivalents, each returning `{ trend, cumulative }` — the deterministic deltas and the synthesized report together, so the UI never has to reconcile two separate fetches into one render pass. The UI renders the two shapes with separate functions (`renderTeamCumulativeReportHtml`/`renderPersonCumulativeReportHtml` in `web/app.js`) rather than one function branching on shape, since the two schemas are genuinely different objects, not variants of one.

**Single-sprint `reason` gets the same analytical depth, but has to deliver it inside the existing `notes` fields, not new JSON keys.** The canonical `TeamReport` schema fixes `team_recommendations` to exactly `{redistribute_work, sprint_feasibility, notes}` — "Claude Code should never alter this structure unless explicitly instructed" (see the canonical schema section above), and this object is saved back into a snapshot's `report_json` verbatim, so it can't grow team_overview/engineer_patterns/leadership_takeaways fields the way the trend schemas did. Instead, `buildPrompt` (`src/reasoning/types.ts`) instructs the model to write `team_recommendations.notes` as a structured markdown write-up using fixed section headings — `## Throughput`, `## Cycle Time`, `## Who Needs Attention`, `## Unassigned` (only when an Unassigned bucket exists), `## Leadership Actions` — landing the same depth and categorization as the trend reports, just scoped to one sprint's data (no cross-sprint volatility/stability narrative, since there's only one data point). This renders correctly with no UI changes needed, since `notes` already goes through `formatMessage` (markdown-aware). Per-engineer `notes` gained a lighter version of the same idea: its first sentence names whether that person reads as overloaded/underutilized/stable *this sprint*, followed by the grounded comparison that shows it.
...
**Known limitation: a small local model can ignore `team_recommendations.notes`'s structured-heading requirement entirely, even with a literal copy-paste skeleton.** Confirmed directly: on the same real saved sprint, `qwen3:8b-q4_K_M` (Ollama) returned a single generic paragraph with zero markdown headings, both before and after adding a second prompt instruction that spelled out the exact `"## Throughput\n[...]\n\n## Cycle Time\n[...]"` skeleton to copy verbatim — while Claude produced the correct four-section structure on the identical prompt/data both times, no skeleton needed. Unlike the `engineer_recommendations`-completeness gap earlier in this doc (fixed by spelling out the exact expected count/names in the prompt body), this one did not respond to the same class of fix — the model isn't missing a count or a name to fill in, it's not attempting section structure at all. This is a genuine model-capability ceiling, not a prompt-wording problem, so there is no further prompt iteration planned here: `reason`/`POST /api/snapshot-reason`/the UI's "Generate Sprint Report" default to Ollama per the rest of this doc, but a user who wants this report's actual depth needs to select Claude — the UI does not currently warn when Ollama is selected that this specific field is likely to come back thin.
...
**A corrected roster weight has to retroactively recompute `load_score`/`burnout_risk`, not just relabel `weight` — the same staleness problem role had, but with an extra dependency role doesn't.** `load_score = rawLoadScore * weight` (`deriveMetrics`), computed once at analyze time and frozen into that snapshot's `report_json` forever. When the roster's weight is corrected later — including via the epoch-backdated roster save, whose whole point is applying "to every sprint for this team, past and future" — an already-saved snapshot's frozen `derived_metrics.weight`/`load_score` doesn't move on its own, exactly like the role-staleness problem documented earlier. The difference: role has no downstream computed value, so overlaying a corrected role live is a pure relabel. Weight does — `load_score`/`burnout_risk` are functions of it — so a first version of this fix that only relabeled the frozen `weight` field left a snapshot showing a corrected weight next to a `load_score` still computed from the OLD one, which is worse than not fixing it at all (the two numbers actively disagree). Caught immediately on a live check against the real production team: the roster showed corrected weights of 3-4 for several engineers, but every saved snapshot's trend data still read `weight: 1` — because the fix hadn't been applied to weight, only role. The real fix (`recomputeWeightedMetrics`/`overlayCurrentRosterFacts`/`overlayCurrentRosterFactsOnReport` in `src/db/teamProfile.ts`) carries each `EngineerMetricPoint`'s own immutable `signals` (the raw, un-weighted work-item facts `deriveMetrics` computes `load_score` FROM) through `computeTrend`, then re-runs the exact same `deriveMetrics` call used at analyze time with the CURRENT roster weight in place of the frozen one — so weight and load_score/burnout_risk are corrected together and stay internally consistent, for the Accumulated Report, the per-person report, and single-sprint reasoning alike. Verified against the real team: an engineer with roster weight 4 (frozen at 1 in every saved snapshot) went from `load_score` 35-41 (reading as the team's most underutilized person) to 140-156 (the correct, weighted figure) the moment this overlay ran — and a live Claude call correctly explained the number as weight-driven rather than volume-driven once it could see the real weight, rather than silently misjudging him as before.
...
**Role, weight, and work-pattern notes can be edited from two places, and both write through the same endpoint.** An earlier version of this UI made the analyzed-report engineer card the *only* editing surface and kept Team Setup's roster table read-only, on the reasoning that the analyzed report is what discovers who's on a team in the first place. In practice that meant setting someone's role required having a source file to re-analyze first — pure friction for a fact that has nothing to do with any particular sprint, and the user explicitly rejected it ("that doesn't make any sense, you should be able [to set] weight and role in Team Setup"). Team Setup's roster table is now directly editable too, not just a read-only overview. (A still-earlier version also had a separate standalone Add/Update Person *form*, disconnected from both of these — that one stays gone, since it required retyping a name from memory instead of picking from who's already known.)

How it actually works now:

- A **Team Setup** panel manages team-level metadata (name, sprint length, charter — `GET /api/teams`, `POST /api/teams/profile` → `createOrUpdateTeam`) **and** the roster table itself.
- Its roster table lists everyone: who's configured, who's been seen in reports but not yet configured (shown as "not yet configured" placeholder text), and (behind a "Show past members" toggle) who's departed. Every row's **Role**, **Weight**, and **Notes** cells are live inputs with their own **Save** button and per-row status line, plus the existing **Mark as departed** action. Editing a field only updates that input's local value; nothing reaches the server until Save is clicked for that row — no save-on-blur, matching the engineer-card convention below.
- **Role, weight, and work-pattern notes are ALSO settable directly on each engineer's card** in the Input panel's analyzed-report view, via an explicit **"Save Role/Weight/Notes to Roster" button per card** — not save-on-blur. This remains the natural place to do it right after seeing someone's numbers, and is still the only way to discover and configure someone who hasn't been typed into Team Setup yet (their name has to come from somewhere real first). Editing a field only updates local display state; nothing reaches the server until that button is clicked. Clicking it reads all three current values off that card and `POST`s them together to `/api/teams/roster` — together, not one field at a time, because the endpoint upserts the whole roster row for that date; saving only the field that changed would silently blow away the other two. A per-card status line directly under the button reports the outcome ("✓ Saved to the ... roster just now." or a clear failure reason) — this is deliberately per-card, not a single shared status message, so it's obvious which person's edit it refers to. **Both editing surfaces post the same three fields together to `/api/teams/roster` for the same reason** — the Team Setup row save reads its own role/weight/notes inputs and sends all three, never just the one field that changed, so editing weight from Team Setup can never silently blank out a notes value set earlier from a card (or vice versa).
...
## Sprint Context (PTO, sprint goal, on-call, cross-team blockers)

Four per-sprint facts the reasoning step needs but the roster can't hold, since the roster describes a person *in general* and these are true for one sprint only:

- **PTO/reduced-capacity** — per-engineer, per-snapshot days out this sprint.
- **On-call/support rotation** — per-engineer, per-snapshot flag.
- **Sprint goal** — team-level, per-snapshot free text: what this sprint was supposed to deliver.
- **Cross-team blocker** — team-level, per-snapshot free text: what external dependency held the sprint back.

**Storage.** The two per-engineer facts live in a new `snapshot_engineer_context` table (`snapshot_id, engineer_name, pto_days, on_call`, upserted on `(snapshot_id, engineer_name)` — setting it twice corrects the fact, it doesn't create a second one, since there's only one true answer per person per snapshot). The two team-level facts are new nullable columns directly on `snapshots` (`sprint_goal`, `blocked_by`) — set at analyze time or edited afterward once known. None of this lives in the roster or the team profile: those are standing facts, this is one-sprint-only, and conflating the two would mean a PTO week from three sprints ago silently "explaining" a snapshot it has nothing to do with.

**CLI.**
```text
teamgauge analyze --team <name> --sprint <label> [--sprint-goal <text>] [--blocked-by <text>] ...
...
teamgauge snapshot rename-sprint --team <name> --from <label> --to <label>
```
`analyze --team` sets the two team-level facts at the moment a snapshot is created. `snapshot set-context`/`set-engineer-context` set or correct any of the four facts on an already-saved snapshot — this is the normal path for PTO/on-call, since a manager usually doesn't know who was out until after the sprint's data is already in. `teamgauge history` includes `sprint_goal`, `blocked_by`, and `engineer_context` in its output.

**Reasoning.** `ReasoningContext` (`src/reasoning/types.ts`) gained `sprintGoal`, `blockedBy`, and `engineerContext: Record<string, {ptoDays?, onCall?}>`. `buildPrompt` folds `pto_days_this_sprint`/`on_call_this_sprint` into each engineer's data (same treatment as `work_pattern_note`) and adds `sprintGoal`/`blockedBy` as top-level ground-truth lines, with explicit instructions: reduced output proportional to PTO days is never underperformance; on-call explains elevated `context_switching_index`/reduced throughput and is a reason NOT to pile more work on that person; a stated `blockedBy` means related slowdowns are attributed to the dependency, not the team. `teamgauge reason --team` auto-loads all four facts for the resolved snapshot, same as it already did for charter/sprint-position/roster notes — verified against a real Claude call: given a team blocked on another team's migration with one engineer on PTO and on-call, the response correctly attributed thin throughput to the external dependency and explicitly declined to flag PTO-adjusted output as a problem, rather than restating the raw low numbers as a concern.

**`POST /api/reason` closed a pre-existing gap while picking this up.** It previously never enriched from the DB at all — even a saved-team analysis got zero charter/sprint-position/roster-notes context, unlike the CLI's `reason --team`. It now optionally accepts `team`/`sprint`/`snapshotDate`; when present, it performs the exact same DB lookup and enrichment (all four sprint-context facts included) before calling the provider. The UI's "Generate Recommendations" button passes these automatically whenever the loaded report came from a saved team (`currentAnalyzedTeam`/`currentSnapshotDate` in `web/app.js`), so this enrichment is not something the user has to ask for separately.

**Server endpoints:** `POST /api/snapshots/context` (`{team, sprint, snapshotDate?, sprintGoal?, blockedBy?}`) and `POST /api/snapshots/engineer-context` (`{team, sprint, snapshotDate?, engineerName, ptoDays?, onCall?}`) mirror the two new CLI commands.

**UI.** The Input panel's saved-team fields gained "Sprint goal" and "Blocked by" text inputs, submitted with the analyze request and displayed read-only in the team summary once loaded (fetched from `/api/history`'s matching snapshot, since these facts live outside the canonical `TeamReport` JSON and aren't part of what `analyze` returns). Each engineer card gained a "PTO days this sprint" number field, an "On-call this sprint" checkbox, and their own **"Save Sprint Context" button — deliberately separate from "Save Role/Weight/Notes to Roster"**, visually divided by a dashed border, because these are a different kind of fact (this-sprint-only vs. standing) persisted to a different place (the snapshot vs. the roster); a single shared save button would blur that distinction the same way folding PTO into roster `notes` would have. Saving requires both `currentAnalyzedTeam` and `currentSnapshotDate` to be set — same "not saved, here's why" messaging as the roster save when the loaded report isn't from a saved team.
